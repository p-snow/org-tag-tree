...
** Ideas
:PROPERTIES:
:CREATED:  [2026-03-06 Fri 13:14]
:END:

Let's suppose automatic tagging with AI for notes in Org mode. Even if AI does not work in a completely deterministic way for tagging, it is very important to provide it with ample information for each tag, so it can tag consistently.

=org-tag-tree= assists users in adding descriptions for each tag in an Org file. Users can submit the Org file as additional context to the AI for more deterministic tagging.
